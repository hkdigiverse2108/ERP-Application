...
class DebitAddEditController extends GetxController {
  ...
  bool isEdit = false;
  DebitNoteModel? existingNote;

  @override
  void onInit() {
    super.onInit();
    final args = Get.arguments;
    if (args != null && args is DebitNoteModel) {
      isEdit = true;
      existingNote = args;
      _initializeData();
    }
  ...
  Future<void> save() async {
    ...
    try {
      final Map<String, dynamic> data = {
        if (isEdit) 'debitNoteId': existingNote!.id,
      ...
      ResModel res;
      if (isEdit) {
        res = await _repo.updateDebitNote(data);
      } else {
        res = await _repo.addDebitNote(data);
      }
