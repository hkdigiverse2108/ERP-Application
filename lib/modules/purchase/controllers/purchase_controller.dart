...
class PurchaseController extends GetxController {
  ...
  Future<void> fetchPurchaseOrders() async {
    try {
      ...
      final String url = ApiConstants.buildUrl(ApiConstants.getAllPurchaseOrder, {
  ...
  Future<void> fetchSupplierBills() async {
    try {
      ...
      final String url = ApiConstants.buildUrl(ApiConstants.getAllSupplierBill, {
        "page": currentPage.value,
        "limit": 10,
        "fromDate": DateFormat('yyyy-MM-dd').format(selectedDateRange.value.start),
        "toDate": DateFormat('yyyy-MM-dd').format(selectedDateRange.value.end),
  ...
  List<dynamic> _extractList(dynamic rawData) {
    if (rawData is List) {
      ...
      if (rawData.containsKey("data") && rawData["data"] is List) {
        return rawData["data"];
      } else if (rawData.containsKey("purchaseOrder_data") && rawData["purchaseOrder_data"] is List) {
        return rawData["purchaseOrder_data"];
      } else if (rawData.containsKey("supplierBill_data") && rawData["supplierBill_data"] is List) {
